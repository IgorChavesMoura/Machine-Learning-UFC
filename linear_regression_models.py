...
def gd(x, y, num_epochs=100, alpha=0.001, w_initial=None, poly_order=1, lamb=None, build_regressors=True, compute_loss=True):
    ...
    for epoch in range(num_epochs):
        ...
        if lamb is None:
            ...
            w += alpha * (np.mean(error[:, None] * x_matrix, axis=0) - lamb * w)
            if compute_loss:
